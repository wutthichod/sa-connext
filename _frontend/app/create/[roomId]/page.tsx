...
export default function RoomParticipantsPageClient() {
  ...
  const fetchEventAndParticipants = async (token: string) => {
    try {
      ...
      const eventsRes = await fetch('/api/events?user=true', {
        headers: {
      ...
      if (participantsRes.ok) {
        ...
        console.log('[Create Event] Participants data:', JSON.stringify(participantsData, null, 2))
        if (participantsData.success && participantsData.data) {
          ...
          console.log('[Create Event] Users array:', JSON.stringify(users, null, 2))
          const mappedParticipants = users.map((u: any) => {
            ...
            console.log('[Create Event] Mapping user:', { raw: u, userId, user_id_field: u.user_id, userId_field: u.userId, id_field: u.id })
            return {
              user_id: userId,
          ...
          console.log('[Create Event] Mapped participants:', mappedParticipants)
      ...
      <Stack direction="row" sx={{ minHeight: '100vh', bgcolor: '#56796F' }}>
        <Box component="main" sx={{ flex: 1, p: 4 }}>
          <Box sx={{ bgcolor: '#fff', p: 6, borderRadius: 1 }}>
            ...
            <Button onClick={() => router.push('/create')} variant="contained">
              Back to Create
            </Button>
          </Box>
        </Box>
        ...
        <Box sx={{ p: 0 }}>
          <Box display="flex" flexDirection="row">
            ...
            <Button
              ...
              disabled={deleting}
              sx={{ bgcolor: '#F1F0CC', height:'60px', width:"12%", alignSelf:"center", color: '#384F52', fontFamily: 'Gloock, sans-serif', fontSize: '1.5rem', textTransform: 'none', mb:3}}
            >
                            ...
                            <IconButton 
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  console.log('[Create Event] Creating chat with user:', recipientId, '(original:', p.user_id, 'type:', typeof p.user_id, ')');
                                  ...
                                  console.log('[Create Event] Chat creation response status:', res.status);

                                  if (res.ok) {
                                    const data = await res.json();
                                    console.log('[Create Event] Chat created successfully:', data);
                                    // Navigate to chat page after creating chat
                                    router.push('/chat');
                                  } else {
                                    const errorData = await res.json().catch(() => ({ error: 'Unknown error' }));
                                    console.error('[Create Event] Chat creation failed:', errorData);
                                    alert(errorData.error || errorData.message || 'Failed to create chat');
                                  }
                                } catch (err: any) {
                                  console.error('[Create Event] Error creating chat:', err);
