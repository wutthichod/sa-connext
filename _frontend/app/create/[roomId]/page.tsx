  ...
  const fetchEventAndParticipants = async (token: string) => {
    try {
      ...
      const participantsRes = await fetch(
        `/api/events/${foundEvent.event_id}/participants`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (participantsRes.ok) {
        ...
        console.log(
          "[Create Event] Participants data:",
          JSON.stringify(participantsData, null, 2)
        );
        if (participantsData.success && participantsData.data) {
          const users = Array.isArray(participantsData.data)
            ? participantsData.data
            : [participantsData.data];
          console.log(
            "[Create Event] Users array:",
          ...
          const mappedParticipants = users.map((u: any) => {
            ...
            console.log("[Create Event] Mapping user:", {
              raw: u,
              userId,
              user_id_field: u.user_id,
              userId_field: u.userId,
              id_field: u.id,
            });
            return {
              user_id: userId,
              ...
              role:
                userId === foundEvent.organizer_id?.toString()
                  ? "Organizer"
                  : "Participant",
            };
          });
          console.log(
            "[Create Event] Mapped participants:",
            mappedParticipants
          );
                    ...
                    participants.map((p) => (
                      <TableRow key={p.user_id} hover>
                        ...
                        <TableCell align="center" sx={{ fontSize: "1rem" }}>
                          ...
                          {/* Only show chat button if not current user */}
                            ...
                            <IconButton
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  console.log(
                                    "[Create Event] Starting chat with user:",
                                    recipientId
                                  );
                                  ...
                                  if (chatsRes.ok) {
                                    ...
                                    if (chatsData.success && chatsData.data) {
                                      ...
                                      const existingChat = chats.find(
                                        (chat: any) =>
                                          !chat.is_group &&
                                          chat.other_participants_id?.includes(
                                            recipientId
                                          )
                                      );
                                      ...
                                      if (existingChat) {
                                        console.log(
                                          "[Create Event] Found existing chat:",
                                          existingChat.chat_id
                                        );
                                        router.push(
                                          `/chat?chatId=${existingChat.chat_id}`
                                        );
                                        return;
                                      }
                                    }
                                  ...
                                  if (res.ok) {
                                    ...
                                    console.log(
                                      "[Create Event] Chat created successfully:",
                                      data
                                    );
                                    ...
                                    console.error(
                                      "[Create Event] Chat creation failed:",
                                      errorData
                                    );
                                    alert(
                                      errorData.error ||
                                  ...
                                  console.error(
                                    "[Create Event] Error creating chat:",
                                    err
                                  );
