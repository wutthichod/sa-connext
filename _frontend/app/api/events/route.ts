...
export async function GET(request: NextRequest) {
  try {
    ...
    const endpoint = userOnly ? `${BACKEND_URL}/events/user` : `${BACKEND_URL}/events`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[API] Create event request body:', JSON.stringify(body, null, 2));
    ...
    try {
      ...
      return NextResponse.json(
        { 
          error: `Failed to connect to backend: ${fetchError.message || 'Connection refused'}. Make sure the backend is running on ${BACKEND_URL}` 
        },
        { status: 503 }
      );
    }
