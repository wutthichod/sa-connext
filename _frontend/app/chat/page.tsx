...
export default function ChatPage() {
  ...
  // Handle URL parameter for auto-selecting chat
  useEffect(() => {
    ...
    if (chatId) {
      console.log("[Chat Page] Auto-selecting chat from URL:", chatId);
      const targetChat = conversations.find((conv) => conv.chat_id === chatId);
      if (targetChat) {
        console.log("[Chat Page] Found target chat:", targetChat.name);
        setSelectedConversation(targetChat);
        if (targetChat.messages.length === 0) {
          fetchMessages(targetChat.chat_id);
        }
        // Remove the chatId parameter from URL
        window.history.replaceState({}, "", "/chat");
      } else {
        console.log("[Chat Page] Chat not found in conversations list");
      }
    }
  }, [conversations]);
  ...
  useEffect(() => {
    ...
    if (lastMessage.success && lastMessage.data) {
      ...
      setSelectedConversation((prev) => {
        ...
        const messageId =
          newMessage._id?.toString() || newMessage.message_id || "";
  ...
  const fetchUserAndChats = async (token: string) => {
    try {
      ...
      if (chatsData.success && chatsData.data) {
        const chats: Chat[] = Array.isArray(chatsData.data)
          ? chatsData.data
          : [chatsData.data];
        ...
        convs.sort((a, b) => {
          ...
          const timeA =
            chatA?.last_message_at ||
            chatA?.updated_at ||
            chatA?.created_at ||
            "";
          const timeB =
            chatB?.last_message_at ||
            chatB?.updated_at ||
            chatB?.created_at ||
            "";
          ...
          {conversations.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="text-xs text-gray-500">
                Start a chat from an event to get started!
              </p>
            </div>
          ) : (
