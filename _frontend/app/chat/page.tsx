...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (selectedConversation && selectedConversation.messages.length > 0 && hydrated) {
      setTimeout(() => {
        const messagesEnd = document.getElementById('messages-end');
        messagesEnd?.scrollIntoView({ behavior: 'smooth' });
      }, 100);
    }
  }, [selectedConversation?.messages.length, hydrated]);

  // handle incoming WebSocket messages
  useEffect(() => {
    if (!hydrated || !lastMessage) return;
    ...
    if (lastMessage.success && lastMessage.data) {
      ...
      setConversations(prev => {
        const updated = prev.map(conv => conv.chat_id === chatId
          ? { ...conv, lastMessage: newMessage.message || 'New message' }
          : conv
        );
  ...
  const fetchUserAndChats = async (token: string) => {
    try {
      ...
      if (chatsData.success && chatsData.data) {
        ...
        const directChats = chats.filter(c => !c.is_group);
        ...
        const convs: Conversation[] = directChats.map(chat => {
          ...
          const initials = participantName.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) || 'U';
  ...
  const fetchMessages = async (chatId: string) => {
    try {
      ...
      if (data.success && data.data) {
        ...
        setSelectedConversation(prev => prev && prev.chat_id === chatId ? { ...prev, messages } : prev);

        setConversations(prev => prev.map(conv => conv.chat_id === chatId ? {
          ...conv,
          messages,
  ...
  if (!hydrated || loading) {
    return (
      <div className="flex h-screen items-center justify-center bg-gradient-to-br from-[#5a7568] to-[#4a6558]">
  ...
  return (
    <div className="flex h-screen bg-[#5a7568] overflow-hidden">
      ...
      <div className="w-72 bg-white border-r border-gray-200 shadow-sm flex flex-col h-full">
        {/* Header */}
        <div className="p-5 border-b border-gray-200 bg-gradient-to-r from-[#5a7568] to-[#4a6558] flex-shrink-0 flex items-center gap-3">
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.length === 0 ? (
            ...
            conversations.map(conv => (
              <div
                key={conv.chat_id}
                className={`w-full border-b border-gray-100 transition-all duration-150 ${selectedConversation?.chat_id === conv.chat_id ? 'bg-[#5a7568]/10 border-l-4 border-l-[#5a7568]' : 'hover:bg-gray-50/50'}`}
              >
                <button onClick={() => handleSelectConversation(conv)} className="w-full p-4 transition-colors text-left">
            ...
            {selectedConversation.messages.length === 0 ? (
              ...
              <div className="space-y-3">
                {selectedConversation.messages.map(msg => {
                  ...
                  const timeString = typeof window !== 'undefined'
                    ? new Date(msg.created_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })
                    : '';

                  return (
                    <div key={msg.message_id} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-2 duration-200`}>
                      <div className="flex items-end gap-2 max-w-md">
                        <div className={`${isUser ? 'bg-gradient-to-br from-[#5a7568] to-[#4a6558] text-white rounded-br-md' : 'bg-white text-gray-900 border border-gray-200 rounded-bl-md'} px-4 py-2.5 rounded-2xl shadow-sm`}>
                          <p className="text-sm leading-relaxed whitespace-pre-wrap break-words">{msg.message}</p>
                          <span className={`${isUser ? 'text-white/70' : 'text-gray-400'} text-[10px] mt-1.5 block`}>{timeString}</span>
                        </div>
                      </div>
                    </div>
                  );
