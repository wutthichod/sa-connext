...
export const WebSocketProvider: React.FC<WebSocketProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const checkAndConnect = () => {
      const token = localStorage.getItem('token') || sessionStorage.getItem('token');
      const currentWs = wsRef.current;
      if (token && (!currentWs || currentWs.readyState === WebSocket.CLOSED || currentWs.readyState === WebSocket.CLOSING)) {
        connect();
      }
    };
  ...
  return (
    <WebSocketContext.Provider value={{ ws, isConnected, sendMessage, lastMessage }}>
      {children}
